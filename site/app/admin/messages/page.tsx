...
import { Trash2, Mail, ArrowLeft, Inbox } from "lucide-react";
        ...
        <div className="rounded-2xl border border-taupe/10 bg-white p-6 shadow-sm">
          ...
          <div className="mt-6 flex gap-3">
            ...
            {confirmDeleteId === selectedMsg.id ? (
              <AdminConfirmDelete
                onConfirm={() => { deleteMessage(selectedMsg.id); setSelected(null); }}
      ...
      <div className="space-y-2">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-2xl border border-taupe/10 bg-white py-16">
            <div className="grid h-14 w-14 place-items-center rounded-2xl bg-taupe/5 text-taupe/30">
              <Inbox className="h-7 w-7" />
            </div>
            <p className="mt-3 font-serif text-lg text-charcoal">No messages</p>
            <p className="text-sm text-taupe">No messages match your search.</p>
          </div>
        ) : (
          filtered.map((msg) => (
            <div
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div
                    className={`grid h-10 w-10 place-items-center rounded-full text-sm font-bold ${
                      msg.read ? "bg-taupe/10 text-taupe" : "bg-gold text-brown"
                    }`}
                  >
                    {msg.name[0]}
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <p className={`text-sm ${msg.read ? "text-charcoal" : "text-charcoal font-semibold"}`}>
                        {msg.name}
                      </p>
                ...
                <div className="text-right">
                  <p className="text-xs text-taupe">{new Date(msg.date).toLocaleDateString()}</p>
                  {confirmDeleteId === msg.id ? (
                    <div className="mt-1">
                      <AdminConfirmDelete
                        onConfirm={(e) => { e?.stopPropagation(); deleteMessage(msg.id); setConfirmDeleteId(null); }}
                        onCancel={(e) => { e?.stopPropagation(); setConfirmDeleteId(null); }}
                      />
                    </div>
                  ) : (
                    <button
